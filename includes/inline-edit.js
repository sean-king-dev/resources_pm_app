...
$.inlineEdit.prototype = {
    ...
    value: function( newValue ) {
        if ( arguments.length ) {
            ...
            this.element.data( 'value' + namespace, $( '.' + placeholderClass, this ).length ? '' : value );
        }
    ...
    controls: {
        ...
        textarea: function( value ) {
            ...
            return '<textarea id="frm_'+id +'" >'+ value.replace(/<br\s?\/?>/g,"\n") +'</textarea>' + this.buttonHtml( { before: '<br />' } );
        },
        
        input: function( value ) {
            var id = this.element[0].id;
            return '<input type="text" id="frm_'+id +'"  value="'+ value.replace(/(\u0022)+/g, '') +'">' + this.buttonHtml();
        }
    },
